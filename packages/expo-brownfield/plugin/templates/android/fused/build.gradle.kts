...
// including the runtime classpaths of `include(project(...))` targets. Scoping to
// just `fusedRuntime` isn't enough — fused-library reads from included projects'
// own configs which our scoped exclude can't touch.
//
// The aggregator (`brownfieldFusedExpoAggregator`) is intentionally exempt so the
// transitive-AAR walk can still record the resolved versions of the excluded
// groups; we inject them into the published POM/.module below as transitive deps
// for the consumer to resolve naturally via Maven/JitPack.
//
// MUST be registered BEFORE the aggregator resolves, otherwise `configureEach`
// throws "Cannot mutate after resolved".
  ...
  tasks.named(metadataTaskName).configure {
    doLast {
      ...
      variants.forEach { variant ->
        ...
        // Annotate react-android / hermes-android edges with this sibling's
        // `fusedVariant` so consumer variant matching pulls the same variant the
        // brownfield's codegen `.so` files (libreact_codegen_*.so, libappmodules.so)
        // were linked against. Otherwise:
        //   - debug-built codegen + release libreactnative.so → unsatisfied link
        //     to debug-only symbols (e.g. ShadowNode::getDebugName, present in
        //     debug builds, stripped from release)
        //   - release-built codegen + debug libreactnative.so → SIGSEGV at
        //     component-descriptor init (struct layout differs)
        deps.forEach { dep ->
      ...
      logger.lifecycle(
        "brownfield.fused[${fusedVariant}]: annotated $annotatedCount dependency edges " +
          "with BuildTypeAttr=${fusedVariant}; stripped $strippedCount conflicting natural-emission " +
